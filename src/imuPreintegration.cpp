...
class TransformFusion : public ParamServer
{
	public:
		...
		// Subcriber and Publisher Nodes
		rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subImuOdometry; // from ImuPreintegration
		rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subLaserOdometry; // from MapOptimization
		...
		TransformFusion(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_transformFusion", options) {
			...
			// Subscribers
			subLaserOdometry = create_subscription<nav_msgs::msg::Odometry>( // from MapOptimization
					"lio_sam/mapping/odometry", qos,
					std::bind(&TransformFusion::lidarOdometryHandler, this, std::placeholders::_1),
					laserOdomOpt);
			subImuOdometry = create_subscription<nav_msgs::msg::Odometry>( // from ImuPreintegration
...
class IMUPreintegration : public ParamServer
{
	public:
		...
		// Subcriber and Publisher Nodes
		rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr subImu; // from IMU
		rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subOdometry; // from MapOptimization
		rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubImuOdometry; // for ImageProjection
		...
		void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
		{
			...
			// 0. initialize system TODO: make this a function call
			...
			// reset graph for speed TODO: make this a function call
			if (key == 100)
			{
